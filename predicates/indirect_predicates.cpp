...
int incircle_fma_filtered(double pax, double pay, double pbx, double pby, double pcx, double pcy, double pdx, double pdy)
{
   ALG16_Double adx = pax - pdx;
   ALG16_Double ady = pay - pdy;
   ALG16_Double bdx = pbx - pdx;
   ALG16_Double bdy = pby - pdy;
   ALG16_Double cdx = pcx - pdx;
   ALG16_Double cdy = pcy - pdy;
   ALG16_Double abdetb = bdx * ady;
   ALG16_Double abdet;
   Explicit_FMSub(adx,bdy,abdetb,abdet);
   ALG16_Double bcdetb = cdx * bdy;
   ALG16_Double bcdet;
   Explicit_FMSub(bdx,cdy,bcdetb,bcdet);
   ALG16_Double cadetb = adx * cdy;
   ALG16_Double cadet;
   Explicit_FMSub(cdx,ady,cadetb,cadet);
   ALG16_Double aliftb = ady * ady;
   ALG16_Double alift;
   Explicit_FMAdd(adx,adx,aliftb,alift);
   ALG16_Double bliftb = bdy * bdy;
   ALG16_Double blift;
   Explicit_FMAdd(bdx,bdx,bliftb,blift);
   ALG16_Double cliftb = cdy * cdy;
   ALG16_Double clift;
   Explicit_FMAdd(cdx,cdx,cliftb,clift);
   ALG16_Double lb = blift * cadet;
   ALG16_Double lab;
   Explicit_FMAdd(alift,bcdet,lb,lab);
   ALG16_Double L;
   Explicit_FMAdd(clift,abdet,lab,L);
   ...
   epsilon *= epsilon;
   epsilon *= epsilon;
   epsilon *= 1.154631945610163e-14;
   if (L > epsilon) return IP_Sign::POSITIVE;
   if (-L > epsilon) return IP_Sign::NEGATIVE;
   return Filtered_Sign::UNCERTAIN;
}

int incircle_fma_interval(interval_number pax, interval_number pay, interval_number pbx, interval_number pby, interval_number pcx, interval_number pcy, interval_number pdx, interval_number pdy)
{
   setFPUModeToRoundUP();
   interval_number adx(pax - pdx);
   interval_number ady(pay - pdy);
   ...
   interval_number cdy(pcy - pdy);
   interval_number abdetb(bdx * ady);
   interval_number abdet(fmsub(adx,bdy,abdetb));
   interval_number bcdetb(cdx * bdy);
   interval_number bcdet(fmsub(bdx,cdy,bcdetb));
   interval_number cadetb(adx * cdy);
   interval_number cadet(fmsub(cdx,ady,cadetb));
   interval_number aliftb(ady * ady);
   interval_number alift(fmadd(adx,adx,aliftb));
   interval_number bliftb(bdy * bdy);
   interval_number blift(fmadd(bdx,bdx,bliftb));
   interval_number cliftb(cdy * cdy);
   interval_number clift(fmadd(cdx,cdx,cliftb));
   interval_number lb(blift * cadet);
   interval_number lab(fmadd(alift,bcdet,lb));
   interval_number L(fmadd(clift,abdet,lab));
   setFPUModeToRoundNEAR();
...
int incircle_fma_exact(double pax, double pay, double pbx, double pby, double pcx, double pcy, double pdx, double pdy)
{
   expansionObject o;
   double adx[2];
   o.two_Diff(pax, pdx, adx);
   ...
   double abdet[16];
   double tvar_1[8];
   int tvar_1_len = o.Gen_Product(2, adx, 2, bdy, tvar_1);
   int abdet_len = o.Gen_Diff(tvar_1_len, tvar_1, abdetb_len, abdetb, abdet);
   double bcdetb[8];
   int bcdetb_len = o.Gen_Product(2, cdx, 2, bdy, bcdetb);
   double bcdet[16];
   double tvar_2[8];
   int tvar_2_len = o.Gen_Product(2, bdx, 2, cdy, tvar_2);
   int bcdet_len = o.Gen_Diff(tvar_2_len, tvar_2, bcdetb_len, bcdetb, bcdet);
   double cadetb[8];
   int cadetb_len = o.Gen_Product(2, adx, 2, cdy, cadetb);
   double cadet[16];
   double tvar_3[8];
   int tvar_3_len = o.Gen_Product(2, cdx, 2, ady, tvar_3);
   int cadet_len = o.Gen_Diff(tvar_3_len, tvar_3, cadetb_len, cadetb, cadet);
   double aliftb[8];
   int aliftb_len = o.Gen_Product(2, ady, 2, ady, aliftb);
   double alift[16];
   double tvar_4[8];
   int tvar_4_len = o.Gen_Product(2, adx, 2, adx, tvar_4);
   int alift_len = o.Gen_Sum(tvar_4_len, tvar_4, aliftb_len, aliftb, alift);
   double bliftb[8];
   int bliftb_len = o.Gen_Product(2, bdy, 2, bdy, bliftb);
   double blift[16];
   double tvar_5[8];
   int tvar_5_len = o.Gen_Product(2, bdx, 2, bdx, tvar_5);
   int blift_len = o.Gen_Sum(tvar_5_len, tvar_5, bliftb_len, bliftb, blift);
   double cliftb[8];
   int cliftb_len = o.Gen_Product(2, cdy, 2, cdy, cliftb);
   double clift[16];
   double tvar_6[8];
   int tvar_6_len = o.Gen_Product(2, cdx, 2, cdx, tvar_6);
   int clift_len = o.Gen_Sum(tvar_6_len, tvar_6, cliftb_len, cliftb, clift);
   double lb_p[128], *lb = lb_p;
   int lb_len = o.Gen_Product_With_PreAlloc(blift_len, blift, cadet_len, cadet, &lb, 128);
   double lab_p[128], *lab = lab_p;
   double tvar_7_p[128], *tvar_7 = tvar_7_p;
   int tvar_7_len = o.Gen_Product_With_PreAlloc(alift_len, alift, bcdet_len, bcdet, &tvar_7, 128);
   int lab_len = o.Gen_Sum_With_PreAlloc(tvar_7_len, tvar_7, lb_len, lb, &lab, 128);
   double L_p[128], *L = L_p;
   double tvar_8_p[128], *tvar_8 = tvar_8_p;
   int tvar_8_len = o.Gen_Product_With_PreAlloc(clift_len, clift, abdet_len, abdet, &tvar_8, 128);
   int L_len = o.Gen_Sum_With_PreAlloc(tvar_8_len, tvar_8, lab_len, lab, &L, 128);
...
int incircle_fma(double pax, double pay, double pbx, double pby, double pcx, double pcy, double pdx, double pdy)
{
   int ret;
   ret = incircle_fma_filtered(pax, pay, pbx, pby, pcx, pcy, pdx, pdy);
   if (ret != Filtered_Sign::UNCERTAIN) return ret;
   ret = incircle_fma_interval(pax, pay, pbx, pby, pcx, pcy, pdx, pdy);
   if (ret != Filtered_Sign::UNCERTAIN) return ret;
   return incircle_fma_exact(pax, pay, pbx, pby, pcx, pcy, pdx, pdy);
}
