...
{
	simde__m128d interval; // interval[1] = min_low, interval[0] = high

	static simde__m128d zero;
	static simde__m128i sign_low_mask, sign_high_mask;
...
public:
	...
	inline interval_number(const double a) : interval(simde_mm_set_pd(-a, a)) {}
	inline interval_number(const double minf, const double sup) : interval(simde_mm_set_pd(minf, sup)) {}
	inline interval_number(const simde__m128d &i) : interval(i) {}
	...
	inline void invert() { interval = simde_mm_shuffle_pd(interval, interval, 1); }

	inline bool isNegative() const { return simde_mm_comilt_sd(interval, zero); }
	inline bool isPositive() const { return simde_mm_comilt_sd(simde_mm_shuffle_pd(interval, interval, 1), zero); }
	...
	inline bool operator<(const double b) const { return (simde_mm_comilt_sd(interval, simde_mm_set_sd(b))); }

	inline interval_number &operator=(const interval_number &b)
	{
		interval = b.interval;
		return *this;
	}

	inline interval_number operator+(const interval_number &b) const { return interval_number(simde_mm_add_pd(interval, b.interval)); }

	inline interval_number operator-(const interval_number &b) const { return interval_number(simde_mm_add_pd(interval, simde_mm_shuffle_pd(b.interval, b.interval, 1))); }

	inline interval_number operator*(const interval_number &b) const
	{
		simde__m128i ssg;
		simde__m128d llhh, lhhl, ip;

		switch ((simde_mm_movemask_pd(interval) << 2) | simde_mm_movemask_pd(b.interval))
		{
		case 0:
			llhh = simde_mm_mul_pd(interval, b.interval);
			lhhl = simde_mm_mul_pd(interval, simde_mm_shuffle_pd(b.interval, b.interval, 1));
			return interval_number(simde_mm_max_pd(simde_mm_unpacklo_pd(llhh, lhhl), simde_mm_unpackhi_pd(llhh, lhhl)));
		case 1:
			return interval_number(simde_mm_mul_pd(simde_mm_shuffle_pd(b.interval, b.interval, 3), simde_mm_shuffle_pd(interval, interval, 1)));
		case 2:
			return interval_number(simde_mm_mul_pd(simde_mm_shuffle_pd(b.interval, b.interval, 0), interval));
		case 4:
			return interval_number(simde_mm_mul_pd(simde_mm_shuffle_pd(interval, interval, 3), simde_mm_shuffle_pd(b.interval, b.interval, 1)));
		case 5:
			ip = simde_mm_mul_pd(simde_mm_castsi128_pd(simde_mm_xor_si128(simde_mm_castpd_si128(interval), sign_high_mask)), b.interval);
			return interval_number(simde_mm_shuffle_pd(ip, ip, 1));
		case 6:
			ssg = simde_mm_xor_si128(simde_mm_castpd_si128(b.interval), sign_low_mask);
			return interval_number(simde_mm_mul_pd(interval, simde_mm_shuffle_pd(simde_mm_castsi128_pd(ssg), simde_mm_castsi128_pd(ssg), 1)));
		case 8:
			return interval_number(simde_mm_mul_pd(simde_mm_shuffle_pd(interval, interval, 0), b.interval));
		case 9:
			ssg = simde_mm_xor_si128(simde_mm_castpd_si128(interval), sign_low_mask);
			return interval_number(simde_mm_mul_pd(b.interval, simde_mm_shuffle_pd(simde_mm_castsi128_pd(ssg), simde_mm_castsi128_pd(ssg), 1)));
		case 10:
			return interval_number(simde_mm_mul_pd(interval, simde_mm_castsi128_pd(simde_mm_xor_si128(simde_mm_castpd_si128(b.interval), sign_low_mask))));
		}
...
public:
	...
	inline void Two_Diff_Back(const double a, const double b, double &x, double &y)
	{
		_bv = a - x;
		y = (a - (x + _bv)) + (_bv - b);
	}
	...
	inline void Double(const int elen, const double *e, double *h) const
	{
		for (int i = 0; i < elen; i++)
			h[i] = 2 * e[i];
	}
	...
	inline static void ExactScale(const int elen, double *e, const double n)
	{
		for (int i = 0; i < elen; i++)
			e[i] *= n;
	}
